...
/***/ (function(module, exports) {
	...
	var config = {
	  apiOrigins: {
	    production: 'https://vast-coast-80096.herokuapp.com',
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onIndexAdvice = function onIndexAdvice(event) {
	  event.preventDefault();
	  $('.error-message').text('');
	  $('.success-message').text('');
	  var data = getFormFields(this);
	  api.showAdvice(data).then(ui.adviceIndexSucces).catch(ui.adviceIndexFail);
	};

	var onAddAdvice = function onAddAdvice(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  api.addAdvice(data).then(ui.adviceAddSuccess).catch(ui.adviceAddFail);
	};

	var onAddDefaultAdvice = function onAddDefaultAdvice() {
	  event.preventDefault();
	  api.addDefaultAdvice().then(ui.adviceAddDefaultSuccess).catch(ui.adviceAddFail);
	};

	var onDeleteAdvice = function onDeleteAdvice() {
	  event.preventDefault();
	  if (store.currentNum >= 0 && store.advices[store.currentNum] !== undefined) {
	    api.deleteAdvice().then(ui.adviceDeleteSuccess).then(function () {
	      api.showAdvice().then(ui.adviceIndexSucces).catch(ui.adviceIndexFail);
	...
	var onEditAdvice = function onEditAdvice(event) {
	  event.preventDefault();
	  var data = getFormFields(this);
	  var newAdvice = data.advice.idea;
	  if (store.currentNum >= 0 && store.advices[store.currentNum] !== undefined) {
	    api.editAdvice(data).then(ui.adviceEditSuccess).then(function (data) {
	...
	var onDeleteAllAdvice = function onDeleteAllAdvice() {
	  event.preventDefault();
	  api.showAdvice().then(ui.adviceIndexSucces).then(function () {
	    for (var i = 0; i < store.advices.length; i++) {
	      api.deleteAllAdvice(store.advices[i].id).then(ui.adviceDeleteAllSuccess).catch(ui.adviceDeleteAllFail);
	    }
	  }).catch(ui.adviceDeleteAllFail);
	};

	var onTakeAdviceNo = function onTakeAdviceNo() {
	  event.preventDefault();
	  if (store.currentNum >= 0 && store.advices[store.currentNum] !== undefined) {
	    api.takeNoAdvice().then(ui.takeNoAdviceSuccess).then(function () {
	      api.showAdvice().then(ui.adviceIndexSucces).catch(ui.adviceIndexFail);
	...
	var onTakeAdviceYes = function onTakeAdviceYes() {
	  event.preventDefault();
	  if (store.currentNum >= 0 && store.advices[store.currentNum] !== undefined) {
	    api.takeYesAdvice().then(ui.takeYesAdviceSuccess).then(function () {
	      api.showAdvice().then(ui.adviceIndexSucces).catch(ui.adviceIndexFail);
	...
	var onTakeAdviceData = function onTakeAdviceData() {
	  event.preventDefault();
	  api.showTakeAdviceData().then(ui.takeAdviceDataSucces).catch(ui.takeAdviceDataFail);
	};

	var onGetExampleAdvice = function onGetExampleAdvice() {
	  event.preventDefault();
	  api.showExampleAdvice().then(ui.exampleSuccess).catch(ui.exampleFail);
	};

	var onRandomUserName = function onRandomUserName() {
	  event.preventDefault();
	  api.getRandomUserName().then(ui.userNameSuccess).catch(ui.userNameFail);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var addAdvice = function addAdvice(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/advices',
	    method: 'POST',
	    controller: 'advices',
	...
	var defaultAdviceArray = ['Take a nap', 'Drink more coffee', 'No candy at the video store', 'Buy a new plant', 'Drink less coffee', 'Call your mom', 'Call your dad', 'Eat some candy', 'Go for a walk', 'Do some push ups', 'Drink some water', 'Do not spend more than you earn', 'Text a friend', 'Clean your room', 'Pet a dog', 'Turn off your phone', 'Wiggle your toes', 'Listen to music you like', 'Read a book'];

	var addDefaultAdvice = function addDefaultAdvice() {
	  var arrayLength = defaultAdviceArray.length;
	  var randNum = Math.floor(Math.random() * arrayLength);
	  var adviceString = defaultAdviceArray[randNum];
	  return $.ajax({
	    url: config.apiOrigin + '/advices',
	    method: 'POST',
	    controller: 'advices',
	...
	var deleteAdvice = function deleteAdvice() {
	  return $.ajax({
	...
	var editAdvice = function editAdvice(data) {
	  return $.ajax({
	    url: config.apiOrigin + '/advices/' + store.advices[store.currentNum].id,
	    method: 'PATCH',
	    controller: 'advices',
	...
	var deleteAllAdvice = function deleteAllAdvice(num) {
	  return $.ajax({
	...
	var takeNoAdvice = function takeNoAdvice() {
	  return $.ajax({
	...
	var takeYesAdvice = function takeYesAdvice() {
	  return $.ajax({
	    url: config.apiOrigin + '/take_advices',
	    method: 'POST',
	    controller: 'advices',
	...
	var showTakeAdviceData = function showTakeAdviceData() {
	  return $.ajax({
	    url: config.apiOrigin + '/take_advices',
	    method: 'GET',
	    headers: {
	...
	var showExampleAdvice = function showExampleAdvice() {
	  return $.ajax({
	...
	module.exports = {
	  ...
	  addAdvice: addAdvice,
	  addDefaultAdvice: addDefaultAdvice,
	  deleteAdvice: deleteAdvice,
	  editAdvice: editAdvice,
	  deleteAllAdvice: deleteAllAdvice,
	  takeNoAdvice: takeNoAdvice,
	  takeYesAdvice: takeYesAdvice,
	  showTakeAdviceData: showTakeAdviceData,
	  showExampleAdvice: showExampleAdvice,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var adviceIndexFail = function adviceIndexFail(response) {
	...
	var adviceIndexSucces = function adviceIndexSucces(response) {
	  store.advices = response.advices;
	  var num = store.advices.length;
	...
	var adviceAddSuccess = function adviceAddSuccess(response) {
	...
	var adviceAddDefaultSuccess = function adviceAddDefaultSuccess(response) {
	  if (added === true) {
	...
	var adviceAddFail = function adviceAddFail(response) {
	  $('#add-advice-error').text('error: please input advice');
	  $('#add-advice-success').text('');
	};

	var adviceDeleteSuccess = function adviceDeleteSuccess(response) {
	  $('.delete-advice').trigger('reset');
	  $('#delete-advice-error').text('');
	  $('#delete-advice-success').text('advice deleted');
	  store.currentNum = -1;
	};
	var adviceDeleteFail = function adviceDeleteFail(response) {
	  $('#delete-advice-error').text('please display advice to delelete');
	  $('#delete-advice-success').text('');
	  store.currentNum = -1;
	};

	var adviceEditSuccess = function adviceEditSuccess(response) {
	  $('.edit_advice').trigger('reset');
	  $('#edit-advice-error').text('');
	  $('#edit-advice-success').text('advice edited');
	};
	var adviceEditFail = function adviceEditFail(response) {
	  $('#edit-advice-error').text('error, please make sure advice is selected and input is valid');
	  $('#edit-advice-success').text('');
	};

	var adviceDeleteAllSuccess = function adviceDeleteAllSuccess(response) {
	...
	var adviceDeleteAllFail = function adviceDeleteAllFail(response) {
	  $('#delete-all-error').text('no advice to delete');
	  $('#delete-all-success').text('');
	};

	var takeNoAdviceSuccess = function takeNoAdviceSuccess(response) {
	  $('#take-advice-error').text('');
	  $('#take-advice-success').text('response successfully recorded');
	};
	var takeNoAdviceFail = function takeNoAdviceFail(response) {
	  $('#take-advice-error').text('You must get advice before you can take it');
	  $('#take-advice-success').text('');
	};

	var takeYesAdviceSuccess = function takeYesAdviceSuccess(response) {
	  $('#take-advice-error').text('');
	  $('#take-advice-success').text('response successfully recorded');
	};
	var takeYesAdviceFail = function takeYesAdviceFail(response) {
	  $('#take-advice-error').text('You must get advice before you can take it');
	  $('#take-advice-success').text('');
	};

	var takeAdviceDataSucces = function takeAdviceDataSucces(response) {
	  var YesNoArray = [];
	  var takeAdvicesArray = response.take_advices;
	  for (var i = 0; i < takeAdvicesArray.length; i++) {
	    YesNoArray.push(takeAdvicesArray[i].yes_or_no);
	  }
	  var totalYes = YesNoArray.reduce(function (prev, curr) {
	    return prev + curr;
	  });
	  var percent = (100 * totalYes / takeAdvicesArray.length).toFixed(2);
	  var howItGoes = 'fine';
	  if (percent >= 90) {
	...
	var takeAdviceDataFail = function takeAdviceDataFail(response) {
	...
	var exampleFail = function exampleFail(response) {
	  $('#get-example-advice-error').text('name generator is still loading, wait and try again later');
	};

	var exampleSuccess = function exampleSuccess(data) {
	...
	var userNameSuccess = function userNameSuccess(data) {
	  var firstName = data.results[0].name.first;
	  var lastName = data.results[0].name.last;
	  var lastNameString = lastName.charAt(0).toUpperCase() + lastName.slice(1);
	  $('#random-user-name-api').text(firstName + lastNameString);
	};

	var userNameFail = function userNameFail(data) {
	...
	module.exports = {
	  ...
	  adviceIndexSucces: adviceIndexSucces,
	  adviceIndexFail: adviceIndexFail,
	  adviceAddSuccess: adviceAddSuccess,
	  adviceAddFail: adviceAddFail,
	  adviceDeleteSuccess: adviceDeleteSuccess,
	  adviceDeleteFail: adviceDeleteFail,
	  adviceEditSuccess: adviceEditSuccess,
	  adviceEditFail: adviceEditFail,
	  adviceAddDefaultSuccess: adviceAddDefaultSuccess,
	  adviceDeleteAllSuccess: adviceDeleteAllSuccess,
	  adviceDeleteAllFail: adviceDeleteAllFail,
	  takeNoAdviceFail: takeNoAdviceFail,
	  takeNoAdviceSuccess: takeNoAdviceSuccess,
	  takeYesAdviceSuccess: takeYesAdviceSuccess,
	  takeYesAdviceFail: takeYesAdviceFail,
	  takeAdviceDataFail: takeAdviceDataFail,
	  takeAdviceDataSucces: takeAdviceDataSucces,
	  exampleFail: exampleFail,
	  exampleSuccess: exampleSuccess,
	  userNameFail: userNameFail,
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "b6da93ea282e96263cfa8f6e10be74fe.png";
